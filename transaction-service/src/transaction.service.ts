...
export class TransactionService {
  ...
  async balanceForUserExists(userUuid: string, balanceUuid: string): Promise<boolean> {
    try {
      return await this.balanceClient.send({ cmd: 'balance-exists' }, { userUuid, balanceUuid }).pipe(timeout(DEFAULT_TIMEOUT)).toPromise();
    } catch (err) {
      throw new RpcException(err);
    }
  }

  async createTransaction(transactionData: CreateTransactionDTO, jwt: string): Promise<Transaction> {
    try {
      const userUuid = await this.getUserUuid(jwt);
      if (!(await this.balanceForUserExists(userUuid, transactionData.balanceUuid))) {
  ...
  async updateTransaction(fields: Prisma.TransactionUpdateInput, jwt: string, transactionUuid: string, balanceUuid: string): Promise<Transaction> {
    try {
      const userUuid = await this.getUserUuid(jwt);
